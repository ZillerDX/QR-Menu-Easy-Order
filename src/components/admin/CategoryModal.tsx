...
import { X, Coffee, Sparkles, CupSoda, Utensils, Cake, Pizza, Heart, Tag, Check } from 'lucide-react';
...
export const CategoryModal: React.FC<CategoryModalProps> = ({
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
      ...
      {/* Modal Dialog Card */}
      <div className="relative w-full max-w-lg bg-white rounded-[28px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-stone-200/80 z-10 flex flex-col max-h-[90vh]">
        {/* 1. Header */}
        ...
        {/* 2. Scrollable Body */}
        <form id="category-form" onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4 text-xs sm:text-sm flex-1 min-h-0">
          <div>
            ...
            <input
              ...
              required
              className="w-full px-4 py-3 rounded-2xl border border-stone-200 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 font-bold text-stone-900 shadow-2xs transition-all"
            />
          </div>
              ...
              {AVAILABLE_ICONS.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-2xl border-2 flex flex-col items-center justify-center gap-1.5 transition-all cursor-pointer active:scale-95 ${
                      isSelected
                        ? 'border-orange-500 bg-orange-50/90 text-orange-600 shadow-sm font-black scale-[1.02]'
                        : 'border-stone-200 bg-white hover:bg-stone-50 text-stone-600 hover:border-stone-300'
                    }`}
                  >
                    <IconComponent className="w-5 h-5" />
                    <span className="text-[11px] truncate max-w-full font-bold">{item.label}</span>
                  </button>
                );
              })}
            </div>
        ...
        {/* 3. Fixed Footer Action Bar (Consistent with all modals) */}
