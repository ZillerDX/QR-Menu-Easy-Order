...
import { 
  ...
  Utensils, Plus, Trash2, Sliders, Layers, Coffee, Zap 
} from 'lucide-react';
...
const PRESET_TEMPLATES: { label: string; iconKey: 'sweet' | 'milk' | 'beans' | 'toppings' | 'egg'; group: () => OptionGroup }[] = [
  {
        ...
        <form id="item-editor-form" onSubmit={handleSubmit} className="p-5 sm:p-6 overflow-y-auto space-y-6 flex-1 min-h-0 text-xs sm:text-sm">
          ...
          <div className="bg-orange-50/40 rounded-3xl p-4 sm:p-5 border border-orange-200/80 space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-1 text-[11px] font-black uppercase tracking-wider text-orange-800">
                <Zap className="w-3.5 h-3.5 text-orange-600 shrink-0" />
                <span>{language === 'th' ? 'เพิ่มด่วนจากเทมเพลตยอดนิยม:' : 'Quick Add from Presets:'}</span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {PRESET_TEMPLATES.map((tmpl, idx) => (
                  <button
                    ...
                    className="px-2.5 py-1 bg-white hover:bg-orange-100/90 border border-orange-200/90 text-stone-800 hover:text-orange-950 font-bold text-xs rounded-xl shadow-2xs transition flex items-center gap-1.5 cursor-pointer active:scale-95"
                  >
                    {tmpl.iconKey === 'sweet' && <Sparkles className="w-3.5 h-3.5 text-amber-500 shrink-0" />}
                    {tmpl.iconKey === 'milk' && <Coffee className="w-3.5 h-3.5 text-blue-500 shrink-0" />}
                    {tmpl.iconKey === 'beans' && <Coffee className="w-3.5 h-3.5 text-amber-700 shrink-0" />}
                    {tmpl.iconKey === 'toppings' && <Plus className="w-3.5 h-3.5 text-orange-500 shrink-0" />}
                    {tmpl.iconKey === 'egg' && <Utensils className="w-3.5 h-3.5 text-emerald-600 shrink-0" />}
                          ...
                          >
                            ...
                            <div className="relative w-24">
                              <input
                                ...
                                min="0"
                                step="5"
                                value={choice.priceDelta === 0 ? '' : choice.priceDelta}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  handleUpdateChoice(gIdx, cIdx, { priceDelta: val === '' ? 0 : Number(val) });
                                }}
                                placeholder="+0"
